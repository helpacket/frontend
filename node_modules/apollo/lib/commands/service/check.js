        ...
        try {
            await this.runTasks(({ config, flags, project }) => {
                ...
                return [
                    {
                        enabled: () => !!serviceName,
                        title: `Validate graph composition for service ${chalk_1.default.cyan(serviceName || "")} on graph ${chalk_1.default.cyan(graphName)}`,
                        task: async (ctx, task) => {
                            if (!serviceName) {
                            ...
                            task.title = `Found ${utils_1.pluralize(compositionValidationResult.errors.length, "graph composition error")} for service ${chalk_1.default.cyan(serviceName)} on graph ${chalk_1.default.cyan(graphName)}`;
                    ...
                    {
                        title: `Validating ${serviceName ? "composed " : ""}schema against tag ${chalk_1.default.cyan(tag)} on graph ${chalk_1.default.cyan(graphName)}`,
                        enabled: () => !serviceName,
                        task: async (ctx, task) => {
