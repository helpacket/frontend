    ...
    async run() {
        ...
        try {
            await this.runTasks(({ config, flags, project }) => {
                ...
                return [
                    {
                        title: `Fetching list of services for graph ${chalk_1.default.cyan(graphName + "@" + variant)}`,
                        task: async (ctx, task) => {
        ...
        if (!service || !taskOutput.config) {
            throw new Error("Service missing from config. This should have been validated elsewhere");
        }
